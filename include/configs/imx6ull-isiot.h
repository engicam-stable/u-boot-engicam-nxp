...
#ifndef CONFIG_BOOT_FROM_EMMC // Boot from SD

	#define CONFIG_BOOTCMD "bootcmd=run bootcmd_mmc\0"
	#define BOOTCMD_FROM_SD \
		"bootargs_mmc=run bootargs_base; setenv bootargs ${bootargs_tmp} root=/dev/mmcblk${mmcdev}p1 rootwait rw\0" \
		"bootcmd_mmc=setenv mmcdev 0; run bootargs_emmc; run loadfdt; run loadzImage; bootz ${loadaddr} - ${fdt_addr}\0"
	#define CONFIG_BOOTCMD		"bootcmd=run bootcmd_mmc\0"
#else
